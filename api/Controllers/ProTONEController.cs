...
namespace OPMedia.API.Controllers
{
    [ApiController]
    [Route("[controller]")]
    public class ProTONEController : ControllerBaseEx
    {
        static readonly Version transitionVersion = new Version("3.1.59");

        public ProTONEController(IWebHostEnvironment hostingEnvironment)
            : base(hostingEnvironment)
        {
        }

        [HttpGet]
        public IActionResult Get([FromQuery]string release, [FromQuery] string version)
        {
            try
            {
                Version v = null;
        ...
        public List<BuildInfo> GetProtoneBuilds(BuildType buildType, Version minVersion)
        {
            ...
            string path = Path.Combine(WebRootPath, $"ProTONE/{folder}");
            ...
            try
            {
                if (System.IO.File.Exists(path))
                {
                    ...
                    bi = new BuildInfo
                    {
                        ...
                        URL = $"{Request.Scheme}://{Request.Host}/ProTONE/{folder}/{fileName}"
                    };
