...
{
    public static class MeteoScaleSettings
    {
        ...
        public static T ReadIniValue<T>(string section, string key, T defValue = default(T))
        {
            ...
            try
            {
                retVal = IniFile.Read(section, key, defValue);
            }
            catch
            {
                retVal = defValue;
    ...
    {
        static string _iniFilePath = null;
        static FileSystemWatcher _watchConfigFile = null;
        static string _content = null;

        static IniFile()
        {
            _iniFilePath = HostingEnvironment.MapPath($"/meteo/data/ScaleSettings.ini");
        ...
        {
            _content = File.ReadAllText(_iniFilePath);
        }

        public static T Read<T>(string section, string key, T defValue = default(T))
        {
            T retVal = defValue;

            try
            {
                using (MemoryStream ms = new MemoryStream(Encoding.ASCII.GetBytes(_content)))
