...
$deployPath = "$hostPath\deploy\beta"
...
function Check-AppZipExists {
    if (Test-Path $appZipFile) {
        ...
        Write-Output "[BETA] APP ZIP archive not found."
        exit 1
    }
}
...
function Check-ApiZipExists {
    if (Test-Path $apiZipFile) {
        ...
        Write-Output "[BETA] API ZIP archive not found."
        exit 1
    }
}
...
function Extract-ApiZip {
    try {
        ...
        Write-Output "[BETA] Failed to extract the API ZIP archive."
        exit 1
    }
}
...
function Copy-Content {
    try {
        ...
        Write-Output "[BETA] Failed to copy content to the website folder."
        exit 1
    }
}
...
function Clean-Up {
    try {
        ...
        Write-Output "[BETA] Failed to clean up."
        exit 1
    }
}
...
Write-Output "[BETA] Deployment completed successfully."